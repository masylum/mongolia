...
module.exports = function (db, collection_name) {
  ...
  MODEL.validateAndUpdate = function (query, update, options, callback) {
    ...
    /*Trying to comment this line to see if solves the validateandupdate ignoring namespace problem*/
  ...
  /**
   * Add some shortcuts for collection methods
   */
  _.each([ 'find', 'findArray', 'insert', 'update', 'remove', 'rename', 'insertAll'
         , 'save', 'distinct', 'count', 'drop', 'findAndModify', 'findAndRemove'
         , 'findOne', 'mapReduce', 'mapReduceArray', 'mapReduceCursor', 'group'
         , 'aggregate', 'stats'], function (method) {
    MODEL[method] = function () {
      MODEL.mongo.apply(MODEL, [method].concat(_.toArray(arguments)));
    };
  });
