...
COLLECTION.insert = function (model, collection, args, callback) {
  args[args.length - 1] = function (error, docs) {
    ...
    if (model.afterCreate.length === 1) {
      callback(null, model.afterCreate(docs));
    } else {
  ...
  if (model.beforeCreate.length === 1) {
    args[0] = model.beforeCreate(args[0]);
...
['findAndModify', 'update'].forEach(function (method) {
  COLLECTION.findAndModify = COLLECTION.update = function (model, collection, args, callback) {
    args[args.length - 1] = function (error, docs) {
      ...
      if (model.afterUpdate.length === 1) {
        callback(null, model.afterUpdate(args[1]));
      } else {
        model.afterUpdate(args[1], callback);
      }
    };

    if (model.beforeUpdate.length === 1) {
      args[1] = model.beforeUpdate(args[1]);
...
COLLECTION.remove = function (model, collection, args, callback) {
  args[args.length - 1] = function (error, _) {
    ...
    if (model.afterRemove.length === 1) {
      callback(null, model.afterRemove(args[0]));
    } else {
  ...
  if (model.beforeRemove.length === 1) {
    args[0] = model.beforeRemove(args[0]);
